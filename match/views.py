...
def recruit_create(request):
    try:
        ...
        return render(request,'match/recruit_error.html',params)
    else:
        params = {'form': form}
        if request.method == 'POST' and form.is_valid():
            post = form.save(commit=False)
...
def profile(request):
    try:
        ...
        return render(request,'match/profile_error.html',params)
...
def error(request):
    ...
    return render(request, 'profile_error.html', contexts, status=404)
"""
def term(request):
